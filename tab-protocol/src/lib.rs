	...
	pub fn parse_message_frame(msg: TabMessageFrame) -> Result<Self, ProtocolError> {
		...
		match header {
			...
			message_header::FRAMEBUFFER_LINK => {
				...
				msg.expect_n_fds(1)?;
				let dma_buf = msg.fds[0];
				Ok(TabMessage::FramebufferLink { payload, dma_buf })
			}
			message_header::FRAME_COMMIT => {
				msg.expect_n_fds(1)?;
				let fence_fd = msg.fds[0];
				let payload = FrameCommitPayload {
					monitor_id: msg.payload.clone().ok_or(ProtocolError::ExpectedPayload)?,
				};
				Ok(TabMessage::FrameCommit { payload, fence_fd })
			}
