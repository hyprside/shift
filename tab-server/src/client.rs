...
use tab_protocol::{
	AuthErrorPayload, AuthOkPayload, ErrorPayload, FramebufferLinkPayload, SessionCreatePayload,
...
impl<Texture> Client<Texture> {
	...
	pub fn handle_message(
		...
		match message {
			...
			TabMessage::FramebufferLink { payload, dma_buf } => {
				match (self.load_texture)(dma_buf, &payload) {
					Ok(texture) => {
						info!(
							client_id = %self.id,
							monitor_id = %payload.monitor_id,
							"Client linked framebuffer"
						);
